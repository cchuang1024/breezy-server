...
class CustomCosyVoice:
    ...
    def inference_zero_shot_no_unit_condition_no_normalize(
        ...
        for i in re.split(r"(?<=[？！。.?!])\s*", tts_text):
            ...
            model_input["llm_prompt_speech_token"] = model_input[
...
def get_bopomofo_rare(text, converter):
    ...
    for i in range(len(text_w_bopomofo)):
        ...
        if (
            word_to_dataset_frequency[t[0]] < 500
            and t[1] != None
            and next_t_char != "["
        ):
            ...
            if (
                t[1] != char2phn[t[0]][0]
                and (
                    word_to_dataset_frequency[t[0]] < 10000
                    or t[0] in always_augment_chars
                )
                and next_t_char != "["
            ):  # Not most common pronunciation
                # Add the char and the pronunciation
                reconstructed_text += t[0] + f"[:{t[1]}]"
            else:
